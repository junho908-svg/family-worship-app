...
export default function FamilyWorship() {
  ...
  // v0.9.8: 헤더 아이콘 클릭 시 안내 모달
  const [pwaModalOpen, setPwaModalOpen] = useState(false);
  // PWA 설치 상태 (이미 설치되어 있는지)
  ...
  // PWA 모달에서 직접 설치 (Android 전용)
  const handlePwaModalInstall = async () => {
    if (pwaInstallEvent) {
      pwaInstallEvent.prompt();
      const { outcome } = await pwaInstallEvent.userChoice;
      if (outcome === 'accepted') {
...
// ============ v0.9.8: PWA 안내 모달 (헤더 아이콘 클릭 시) ============
function PwaInstallModal({ installEvent, onInstall, onClose }) {
  // 플랫폼 감지
  const isIOS = /iPhone|iPad|iPod/.test(navigator.userAgent) && !window.MSStream;
  const isAndroid = /Android/.test(navigator.userAgent);
  ...
  return (
    <div
      className="fixed inset-0 flex items-center justify-center z-[100] anim-fade-up"
      style={{
    ...
    >
      <div
        className="paper-card rounded-3xl w-full max-w-[400px] relative"
        style={{
          background: 'linear-gradient(135deg, #FFFEF9, #FFF8EC)',
      ...
      >
        ...
        <button
          ...
          aria-label="닫기"
        >
          <span className="text-[#8B5E3C] text-xl leading-none" style={{ marginTop: '-2px' }}>×</span>
        </button>

        {/* 로고몽 */}
        <div className="flex justify-center mb-4">
          <Logomong size={96} animate="bounce" glow variant="welcome" />
        </div>

        {/* 제목 */}
        <h2
          className="font-display text-xl font-bold text-center text-[#4A3F35] mb-2"
        ...
        {/* 설명 */}
        <p
          className="text-sm text-[#6B5640] text-center mb-5 leading-relaxed"
          style={{ wordBreak: 'keep-all' }}
        >
          앱처럼 편하게 사용하실 수 있어요!<br/>
          한 번 추가하면 빠르게 시작하실 수 있어요.
        </p>
        ...
        {isAndroid && installEvent && (
          <>
            <button
              onClick={onInstall}
              className="w-full py-3 rounded-2xl font-display text-base font-bold text-white transition-all active:scale-95 mb-3"
              style={{
                background: 'linear-gradient(135deg, #E9A94D, #D4756B)',
                boxShadow: '0 6px 16px -4px rgba(212, 117, 107, 0.5)'
              }}
            >
              ✨ 지금 추가하기
            </button>
            <p className="text-[11px] text-[#8B5E3C] text-center opacity-80">
              버튼을 누르면 안드로이드가 자동으로 홈 화면에 추가해드려요
            </p>
          </>
        )}

        {/* === Android: 자동 설치 불가 시 (Samsung Internet 등) === */}
        {isAndroid && !installEvent && (
          <div className="space-y-3">
            <div className="rounded-2xl p-4" style={{ background: '#FFF8EC', border: '1px solid #F0E2C6' }}>
              <div className="font-display font-bold text-[#4A3F35] mb-2 text-sm">
                📱 안드로이드 사용 방법
              </div>
              <ol className="text-xs text-[#6B5640] space-y-2 leading-relaxed" style={{ wordBreak: 'keep-all' }}>
                <li>
                  <span className="font-bold text-[#D4756B]">1.</span>{' '}
                  브라우저 우측 상단 <span className="font-bold">⋮ (점 3개)</span> 메뉴 탭
                </li>
                <li>
                  <span className="font-bold text-[#D4756B]">2.</span>{' '}
                  <span className="font-bold">"홈 화면에 추가"</span> 또는{' '}
                  <span className="font-bold">"앱 설치"</span> 선택
                </li>
                <li>
                  <span className="font-bold text-[#D4756B]">3.</span>{' '}
                  <span className="font-bold">"추가"</span> 버튼 탭하면 완료!
                </li>
              </ol>
            </div>
            {isSamsung && (
              <p className="text-[11px] text-[#8B5E3C] text-center opacity-80" style={{ wordBreak: 'keep-all' }}>
                💡 삼성 인터넷에서는 ⋮ 메뉴 → "현재 페이지 추가" 를 선택하세요
              </p>
            )}
          </div>
        )}
          ...
          <div className="rounded-2xl p-4" style={{ background: '#FFF8EC', border: '1px solid #F0E2C6' }}>
            ...
            <ol className="text-xs text-[#6B5640] space-y-2 leading-relaxed" style={{ wordBreak: 'keep-all' }}>
              <li>
                <span className="font-bold text-[#D4756B]">1.</span>{' '}
                Safari 하단의{' '}
                <span className="font-bold">⬆️ 공유 버튼</span> 탭
              </li>
              <li>
                <span className="font-bold text-[#D4756B]">2.</span>{' '}
                메뉴 스크롤 →{' '}
            ...
            <p className="text-[11px] text-[#8B5E3C] mt-3 opacity-80" style={{ wordBreak: 'keep-all' }}>
              💡 반드시 Safari로 접속하셔야 해요 (Chrome ❌)
            </p>
          </div>
        )}
          ...
          <div className="rounded-2xl p-4" style={{ background: '#FFF8EC', border: '1px solid #F0E2C6' }}>
            ...
            <ol className="text-xs text-[#6B5640] space-y-2 leading-relaxed" style={{ wordBreak: 'keep-all' }}>
              <li>
                <span className="font-bold text-[#D4756B]">1.</span>{' '}
                Chrome 주소창 우측의{' '}
                <span className="font-bold">⊕ 또는 설치 아이콘</span> 탭
              </li>
              <li>
                <span className="font-bold text-[#D4756B]">2.</span>{' '}
                <span className="font-bold">"설치"</span> 버튼 클릭
              </li>
              <li>
                <span className="font-bold text-[#D4756B]">3.</span>{' '}
                바탕화면이나 시작 메뉴에 추가됨!
        ...
        {/* 하단 안내 */}
        <p className="text-[10px] text-[#8B5E3C] text-center mt-4 opacity-60" style={{ wordBreak: 'keep-all' }}>
          🐑 가족분들 핸드폰에도 똑같이 추가하실 수 있어요
        </p>
      </div>
    </div>
  );
    ...
    <div className="flex items-center justify-between py-4 anim-fade-up">
      ...
      <div className="flex items-center gap-1.5">
        ...
        {!isPwaInstalled && onPwaIconClick && (
          <button
            onClick={onPwaIconClick}
            className="flex items-center justify-center w-9 h-9 rounded-full paper-card transition-all active:scale-90"
            title="홈 화면에 추가하기"
            aria-label="홈 화면에 추가하기"
          >
            <span className="text-base" style={{ filter: 'drop-shadow(0 1px 1px rgba(0,0,0,0.1))' }}>📲</span>
          </button>
        )}
        {/* v0.9.5: 글씨 크기 토글 버튼 */}
        {onFontSizeToggle && (
        ...
        <div className="flex items-center gap-1 px-2.5 py-1.5 rounded-full paper-card">
